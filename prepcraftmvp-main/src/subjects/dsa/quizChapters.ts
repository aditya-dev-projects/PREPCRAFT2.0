const chapter1Quiz = {
  id: 1,
  title: 'Basics Of Programming',
  subchapters: [
    { id: 'variables-data-types-quiz', title: 'Variables & Data Types Quiz', quizId: 'variables-data-types-quiz' },
    { id: 'conditional-statements-quiz', title: 'Conditional Statements Quiz', quizId: 'conditional-statements-quiz' },
    { id: 'operators-quiz', title: 'Operators Quiz', quizId: 'operators-quiz' },
    { id: 'loops-quiz', title: 'Loops Quiz', quizId: 'loops-quiz' },
    { id: 'functions-quiz', title: 'Functions Quiz', quizId: 'functions-quiz' },
  ],
};

const chapter2Quiz = {
  id: 2,
  title: 'Arrays',
  subchapters: [
    { id: 'array-basics-quiz', title: 'Array Basics Quiz', quizId: 'array-basics-quiz' },
    { id: '2d-arrays-quiz', title: '2D Arrays Quiz', quizId: '2d-arrays-quiz' },
    { id: 'strings-quiz', title: 'Strings Quiz', quizId: 'strings-quiz' },
  ],
};

const chapter3Quiz = {
  id: 3,
  title: 'Sorting Techniques',
  subchapters: [
    { id: 'what-is-sorting-quiz', title: 'What Is Sorting Quiz', quizId: 'what-is-sorting-quiz' },
    { id: 'bubble-sort-quiz', title: 'Bubble Sort Quiz', quizId: 'bubble-sort-quiz' },
    { id: 'selection-sort-quiz', title: 'Selection Sort Quiz', quizId: 'selection-sort-quiz' },
    { id: 'insertion-sort-quiz', title: 'Insertion Sort Quiz', quizId: 'insertion-sort-quiz' },
    { id: 'counting-quiz', title: 'Counting Quiz', quizId: 'counting-quiz' },
  ],
};

const chapter4Quiz = {
  id: 4,
  title: 'Problem Solving Techniques',
  subchapters: [
    { id: 'recursion-quiz', title: 'Recursion Quiz', quizId: 'recursion-quiz' },
    { id: 'backtracking-quiz', title: 'Backtracking Quiz', quizId: 'backtracking-quiz' },
    { id: 'divide-and-conqueror-quiz', title: 'Divide & Conqueror Quiz', quizId: 'divide-and-conqueror-quiz' },
    { id: 'bit-manipulation-quiz', title: 'Bit Manipulation Quiz', quizId: 'bit-manipulation-quiz' },
    { id: 'time-and-space-complexity-quiz', title: 'Time & Space Complexity Quiz', quizId: 'time-and-space-complexity-quiz' },
    { id: 'greedy-algorithim-quiz', title: 'Greedy Algorithm Quiz', quizId: 'greedy-algorithim-quiz' },
  ],
};

const chapter5Quiz = {
  id: 5,
  title: 'Object Oriented Programming',
  subchapters: [
    { id: 'basic-of-oops-quiz', title: 'Basic Of OOPs Quiz', quizId: 'basic-of-oops-quiz' },
    { id: 'advanced-oops-quiz', title: 'Advanced OOPs Quiz', quizId: 'advanced-oops-quiz' },
  ],
};

const chapter6Quiz = {
  id: 6,
  title: 'Linear Data Structures',
  subchapters: [
    { id: 'array-lists-quiz', title: 'Array Lists Quiz', quizId: 'array-lists-quiz' },
    { id: 'linked-lists-quiz', title: 'Linked Lists Quiz', quizId: 'linked-lists-quiz' },
    { id: 'stacks-quiz', title: 'Stacks Quiz', quizId: 'stacks-quiz' },
    { id: 'queues-quiz', title: 'Queues Quiz', quizId: 'queues-quiz' },
  ],
};

const chapter7Quiz = {
  id: 7,
  title: 'Trees',
  subchapters: [
    { id: 'binary-trees-quiz', title: 'Binary Trees Quiz', quizId: 'binary-trees-quiz' },
    { id: 'binary-search-trees-quiz', title: 'Binary Search Trees (BST) Quiz', quizId: 'binary-search-trees-quiz' },
  ],
};

const chapter8Quiz = {
  id: 8,
  title: 'Advanced Data Structures',
  subchapters: [
    { id: 'heaps-priority-queues-quiz', title: 'Heaps/Priority Queues Quiz', quizId: 'heaps-priority-queues-quiz' },
    { id: 'hashing-maps-sets-quiz', title: 'Hashing (Maps & Sets) Quiz', quizId: 'hashing-maps-sets-quiz' },
    { id: 'tries-quiz', title: 'Tries Quiz', quizId: 'tries-quiz' },
    { id: 'graphs-quiz', title: 'Graphs Quiz', quizId: 'graphs-quiz' },
    { id: 'segment-trees-quiz', title: 'Segment Trees Quiz', quizId: 'segment-trees-quiz' },
  ],
};

const chapter9Quiz = {
  id: 9,
  title: 'Dynamic Programming',
  subchapters: [
    { id: 'basic-dp-quiz', title: 'Basic DP Quiz', quizId: 'basic-dp-quiz' },
    { id: 'advance-dp-quiz', title: 'Advance DP Quiz', quizId: 'advance-dp-quiz' },
  ],
};

export const dsaQuizChapters = [
  chapter1Quiz,
  chapter2Quiz,
  chapter3Quiz,
  chapter4Quiz,
  chapter5Quiz,
  chapter6Quiz,
  chapter7Quiz,
  chapter8Quiz,
  chapter9Quiz,
];